<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    <body>
        ...
        <form action="ResetPwd.do" method="post">
            <fieldset>
                <legend>會員信息驗證</legend>
                <label for="name">姓名:&nbsp;</label><input type="text" name="userName" id="name" required /><br>

                <label for="date">出生年月日:&nbsp;</label><input type="text" name="userbirthDate" id="date" required
                    placeholder="2000/01/01" autocomplete="off" /><span id="warn_date"></span><br>
                <p>(格式:西元年/月/日[yyyy/MM/dd])</p>

                <label for="sex">性別:&nbsp;</label>
                <label>
                    <input type="radio" name="userGender" value="M" id="" required checked>男
                </label>
                <label>
                    <input type="radio" name="userGender" value="F" id="">女
                </label><br>

                <label for="mail">信箱:&nbsp;</label><input type="text" name="userMail" id="mail" required />
                <span id="warn_mail"></span><br>

                <label for="acc">帳號:&nbsp;</label><input type="text" name="userAccount" id="acc" required /><br>

            </fieldset>
            <fieldset>
                <legend>重設密碼</legend>

                <label for="pwd">密碼:&nbsp;</label><input type="password" name="userPwd" id="pwd" required
                    autocomplete="off" />
                <span id="warn_pwd"></span><br>
                <p>(1.不可空白&nbsp;2.至少6個字且必須包含英數字、特殊字元[!@#$%^&*`])&nbsp;3.最多20個字</p>

                <label for="pwd">再次輸入密碼:&nbsp;</label><input type="password" id="checkPassword" name="checkPassword"
                    onblur="validateDate()" required /><br>

            </fieldset>
            <br>
            <input type="submit" id="submitButton" value="重設密碼" />
        </form>
        ...
        <script>
            ...
            document.getElementById("mail").addEventListener("blur", function () {
                ...
                let printMailWarn = document.getElementById("warn_mail")
                let flag_mail = false;
            ...
            function check_date() {
                ...
                if (dateValue == "")
                    ...
                    if (year == date_check_y && month == date_check_m && day == date_check_d)
